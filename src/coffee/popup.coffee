class DownloadsView
  ...
  updateFileDom: ($fileDom, file) ->
    ...
    if file.size
      $('.file-size', $fileDom).text(humanize.filesize(file.size)).attr('title', humanize.numberFormat(file.size, 0))
    ...
    if state >= DropshipFile.UPLOADED or state < DropshipFile.DOWNLOADING
      ...
      if file.size
        $('.file-item-progress', $fileDom).attr 'max', file.size * (@downloadProgressWeight + @saveProgressWeight + @uploadProgressWeight)
      else
        $('.file-item-progress', $fileDom).removeAttr 'max'

      $('.file-item-progress', $fileDom).attr 'value', file.downloadedBytes() * @downloadProgressWeight + file.savedBytes() * @saveProgressWeight + file.uploadedBytes() * @uploadProgressWeight

      if state >= DropshipFile.UPLOADING
        $('.file-item-progress', $fileDom).attr 'title', "#{humanize.numberFormat(file.uploadedBytes(), 0)} / " + "#{humanize.numberFormat(file.size, 0)} bytes uploaded to dropbox"
        iconClass = 'icon-upload'
      else if file.state() >= DropshipFile.SAVING
        $('.file-item-progress', $fileDom).attr 'title', "#{humanize.numberFormat(file.savedBytes(), 0)} / " + "#{humanize.numberFormat(file.size, 0)} bytes saved to disk"
        iconClass = 'icon-hdd'
      else
        $('.file-item-progress', $fileDom).attr 'title', "#{humanize.numberFormat(file.downloadedBytes(), 0)} / " + "#{humanize.numberFormat(file.size, 0)} bytes downloaded"
    ...
    if file.canBeRetried()
      $('.file-item-retry', $fileDom).removeClass('hidden').removeAttr 'disabled'
    else
      $('.file-item-retry', $fileDom).addClass 'hidden'
    if file.canBeCanceled()
      $('.file-item-cancel', $fileDom).removeClass('hidden').removeAttr 'disabled'
    else
      $('.file-item-cancel', $fileDom).addClass 'hidden'
    if file.canBeHidden()
      $('.file-item-hide', $fileDom).removeClass('hidden').removeAttr 'disabled'
    else
